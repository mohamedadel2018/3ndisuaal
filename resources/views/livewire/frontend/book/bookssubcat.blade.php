<x-slot name="title">@lang('app.anddy_soaal') | @lang('app.book_sub_cats') | @lang('app.books')</x-slot>
<x-slot name="meta_description">@lang('app.anddy_soaal') @lang('app.book_sub_cats_meta_description')
        ...
        <div class="contasiner mx-auto bg-color from-gray-800 to-gray-700 flex">
            <div class="w-1/2">
                <div class="flex justify-center items-center h-full text-3xl font-bold text-gray-100">
                    @lang('app.book_sub_cats_books')
                </div>
            </div>
            <div class="w-1/2">
                <div>
                    <img src="{{ asset('images/library.png') }}" alt="library" class="w-full h-60" alt="@lang('app.alt_tag')" />
                </div>
            </div>
        </div>
        {{-- PAGE COVER END --}}
        <div class="container mx-auto text-right">
            <h1 class="page-title p-3 flex justify-end items-center text-md font-bold mx-5  border-b border-gray-300">
                : &nbsp; <a href="javascript:void(0);" class="text-gray-800">{{ $bookSubCat->translate('name') }}</a>
                <strong class=" font-bold text-purple mx-4">
                    < </strong>
                        <a href="{{ route('books', [$bookSubCat->bookCat->id]) }}"
                            class="hover:text-gray-800 text-gray-400">{{ $bookSubCat->bookCat->translate('name') }}</a>
                        <strong class=" font-bold text-purple mx-4">
                            < </strong>
                                <a href="{{ route('encyclopedia.cat') }}"
                                    class="hover:text-gray-800 text-gray-400">@lang('app.encyclopedia_cats')</a>
                                <strong class=" font-bold text-purple mx-4">
                                    < </strong>
                                        <a href="{{ url('/') }}"
                                            class="hover:text-gray-800 text-gray-400">@lang('app.home')</a>
            </h1>
        </div>
            ...
            <div class="w-4/6  p-5">
                <div class="flex justify-end align-center pt-4 pb-5">
                    <input type="text" placeholder="@lang('app.search')"
                        class="border text-right border-gray-400 focus:ring-2 focus:outline-none focus:ring-gray-500 focus:border-gray-500 focus:shadow-lg px-2 py-2 rounded-md text-right shadow-md"
                        wire:model="search">
                </div>
                        ...
                        <div class="w-1/3">
                            <div class="m-3 border border-gray-400 border-green">
                                <a href="{{ route('book', [$book->id, $book->translate('slug')]) }}">
                                    ...
                                    <h4 class="bg-gray-800 text-white text-center py-2">{{ $book->translate('title') }}
                                    </h4>
                                </a>
                            </div>
                        </div>
