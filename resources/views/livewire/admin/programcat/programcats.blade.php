...
<div class="w-100">
    <div class="d-flex justify-content-between align-items-center">
        @if (admin()->can('program.cat.create'))
            <a href="{{ route('admin.programcat.create') }}" class="btn btn-success waves-effect waves-light">
    ...
    <hr>
    {{-- @include('livewire.admin.programcat.edit') --}}
    {{-- @include('livewire.admin.programcat.create') --}}
    @include('alerts')
    <div class="table-responsive">
        <table class="table mb-0 table-bordered table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <th scope="row">{{ $programCat->id }}</th>
                    <th scope="row">{{ $programCat->translate('name') }}</th>
                    <th>
                        @if (admin()->can('program.cat.edit'))
                            <a title="@lang('app.edit')" href="{{ route('admin.programcat.edit',[$programCat->id]) }}"
                                class="btn btn-primary waves-effect waves-light edit-btn">
                                <i class="la la-edit fz-16"></i>
                            </a>
                        @endif
                      @if (admin()->can('program.cat.view'))
                            <a href="{{ route('admin.programcat',[$programCat->id]) }}" title="@lang('app.details')"
                                class="btn btn-warning waves-effect waves-dark text-black-50">
                                <i class="mdi mdi-eye-circle fz-18"></i>
                            </a>
                      @endif
                      @if (admin()->can('program.cat.delete'))
                            <button title="@lang('app.delete')" type="button"
                                class="btn btn-danger waves-effect waves-dark delete-btn" onclick="deleteConfirmation({{ $programCat->id }})">
                                {{-- wire:click="delete({{ $programCat->id }})" --}}
