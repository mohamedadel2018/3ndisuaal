<x-slot name="title">@lang('app.dashboard') | @lang('app.book_cats')</x-slot>
<x-slot name="pageTitle">@lang('app.books') - @lang('app.categories')</x-slot>
<x-slot name="breadcrumb">
    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">@lang('app.dashboard')</a></li>
    <li class="breadcrumb-item active">@lang('app.book_cats')</li>
</x-slot>
<div class="w-100">
    <div class="d-flex justify-content-between align-items-center">
       @if (admin()->can('book.cat.create'))
            <a href="{{ route('admin.bookcat.create') }}" class="btn btn-success waves-effect waves-light" >
    ...
    <hr>
    {{-- @include('livewire.admin.bookcat.edit') --}}
    {{-- @include('livewire.admin.bookcat.create') --}}
    @include('alerts')
    <div class="table-responsive">
        <table class="table mb-0 table-bordered table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <th scope="row">{{ $bookCat->id }}</th>
                    <th scope="row">{{ $bookCat->translate('name') }}</th>
                    <th>
                       @if (admin()->can('book.cat.edit'))
                            <a  title="@lang('app.edit')" href="{{ route('admin.bookcat.edit',[$bookCat->id]) }}"
                                class="btn btn-primary waves-effect waves-light edit-btn">
                                <i class="la la-edit fz-16"></i>
                            </a>
                       @endif
                       @if (admin()->can('book.cat.view'))
                            <a href="{{ route('admin.bookcat',[$bookCat->id]) }}" title="@lang('app.details')"
                                class="btn btn-warning waves-effect waves-dark text-black-50">
                                <i class="mdi mdi-eye-circle fz-18"></i>
                            </a>
                       @endif
                       @if (admin()->can('book.cat.delete'))
                            <button title="@lang('app.delete')" type="button"
                                class="btn btn-danger waves-effect waves-dark delete-btn"
                                onclick="deleteConfirmation({{ $bookCat->id }})">
                                {{-- wire:click="delete({{ $bookCat->id }})" --}}
