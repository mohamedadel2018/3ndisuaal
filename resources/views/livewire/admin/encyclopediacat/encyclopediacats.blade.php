<x-slot name="title">@lang('app.dashboard') | @lang('app.encyclopedia_cats')</x-slot>
<x-slot name="pageTitle">@lang('app.encyclopedia') - @lang('app.categories')</x-slot>
...
<div class="w-100">
    <div class="d-flex justify-content-between align-items-center">
        @if (admin()->can('encyclopedia.cat.create'))
            <a href="{{ route('admin.encyclopediacat.create') }}" class="btn btn-success waves-effect waves-light">
    ...
    <hr>
    {{-- @include('livewire.admin.encyclopediacat.edit')
    @include('livewire.admin.encyclopediacat.create') --}}
    @include('alerts')
    <div class="table-responsive">
        <table class="table mb-0 table-bordered table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <th scope="row">{{ $encyclopediaCat->id }}</th>
                    <th scope="row">{{ $encyclopediaCat->translate('name') }}</th>
                    <th>
                       @if (admin()->can('encyclopedia.cat.edit'))
                            <a title="@lang('app.edit')" href="{{ route('admin.encyclopediacat.edit',[$encyclopediaCat->id]) }}"
                                class="btn btn-primary waves-effect waves-light edit-btn">
                                <i class="la la-edit fz-16"></i>
                            </a>
                       @endif
                        @if (admin()->can('encyclopedia.cat.view'))
                            <a href="{{ route('admin.encyclopediacat',[$encyclopediaCat->id]) }}" title="@lang('app.details')"
                                class="btn btn-warning waves-effect waves-dark text-black-50">
                                <i class="mdi mdi-eye-circle fz-18"></i>
                            </a>
                        @endif
                        @if (admin()->can('encyclopedia.cat.delete'))
                            <button title="@lang('app.delete')" type="button"
                                class="btn btn-danger waves-effect waves-dark delete-btn"
                                onclick="deleteConfirmation({{ $encyclopediaCat->id }})">
                                {{-- wire:click="delete({{ $encyclopediaCat->id }})" --}}
