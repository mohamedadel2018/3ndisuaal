...
<x-slot name="breadcrumb">
    ...
    <li class="breadcrumb-item active">@lang('app.book_sub_cats')</li>
</x-slot>
<div class="w-100">
    <div class="d-flex justify-content-between align-items-center">
       @if (admin()->can('book.subcat.create'))
            <a href="{{ route('admin.booksubcat.create') }}" class="btn btn-success waves-effect waves-light">
    ...
    <hr>
    {{-- @include('livewire.admin.booksubcat.edit') --}}
    {{-- @include('livewire.admin.booksubcat.create') --}}
    @include('alerts')
    <div class="table-responsive">
        <table class="table mb-0 table-bordered table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <th scope="row">{{ $bookSubCat->id }}</th>
                    <th scope="row">{{ $bookSubCat->translate('name') }}</th>
                    <th scope="row">{{ $bookSubCat->bookCat->translate('name') }}</th>
                    <th>
                      @if (admin()->can('book.subcat.edit'))
                            <a  title="@lang('app.edit')" href="{{ route('admin.booksubcat.edit',[$bookSubCat->id]) }}"
                                class="btn btn-primary waves-effect waves-light edit-btn" >
                                <i class="la la-edit fz-16"></i>
                            </a>
                      @endif
                       @if (admin()->can('book.subcat.view'))
                            <a href="{{ route('admin.booksubcat',[$bookSubCat->id]) }}" title="@lang('app.details')"
                                class="btn btn-warning waves-effect waves-dark text-black-50">
                                <i class="mdi mdi-eye-circle fz-18"></i>
                            </a>
                       @endif
                     @if (admin()->can('book.subcat.delete'))
                            <button title="@lang('app.delete')" type="button"
                                class="btn btn-danger waves-effect waves-dark delete-btn"
                                onclick="deleteConfirmation({{ $bookSubCat->id }})">
                                {{-- wire:click="delete({{ $bookSubCat->id }})" --}}
